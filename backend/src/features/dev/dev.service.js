...
export const resetUserRole = async (username, role, clearCreatorProfile) => {
  const { data: user, error: userErr } = await supabase.from('users').select('id').eq('username', username).single()

  if (userErr || !user) throw { statusCode: 404, code: 'NOT_FOUND', message: `User '${username}' not found` }

  const { error } = await supabase.from('users').update({ user_type: role }).eq('id', user.id)
